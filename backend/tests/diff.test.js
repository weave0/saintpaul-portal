...
// Mock BuildingSpec.find for added/removed specs minimal fields
jest.mock('../models/BuildingSpec', () => ({
  find: jest.fn((query, projection) => ({
    lean: () => {
      // Determine which IDs are requested
      const ids = query._id?.$in || [];
      const pool = [...fromSpecs, ...toSpecs];
      return Promise.resolve(pool.filter(s => ids.includes(s._id)).map(s => ({
        _id: s._id,
        name: s.name,
...
// Mock ReconstructionSnapshot.findById with populate chain
...
beforeAll(() => {
  // Ensure a fresh module graph so mocks apply
  jest.resetModules();
  app = require('../server');
});

describe('Snapshot diff endpoint (temporarily skipped; migrate to integration test)', () => {
